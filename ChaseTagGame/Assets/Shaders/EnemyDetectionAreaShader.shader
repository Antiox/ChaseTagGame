...
Shader "Universal Render Pipeline/NiloCat Extension/Screen Space Decal/Unlit"
{
    Properties
    {
        ...
        _GradiantMap("Grandiant Map", 2D) = "white" {}
        [HDR]_Color("_Color (default = 1,1,1,1)", color) = (1,1,1,1)
        [PerRendererData]_Cutoff("Angle", Range(0,360)) = 360
...
#endif
                half4 gradiant = tex2D(_GradiantMap, uv);
                if (gradiant.r > _Cutoff / 360) col.a = 0;
                return col;
            }
            ENDHLSL
        }
